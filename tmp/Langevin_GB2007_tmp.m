...
function [eqs_det1, eqs_det2, eqs_stoch, aux] = getEqs(params)  
    C = @(t, vars) (params(4)/(1 + params(10)/Ca + params(9)*params(10)/(Ca^2) + params(8)*params(9)*params(10)/(Ca^3) + params(7)*params(8)*params(9)*params(10)/(Ca^4)));
    S0 = @(t, vars) (B1 + B2 + B3 + B4 + B5 + B6 + B7 + B8 + B9 + B10 + B11 + B12 + B13);
    B0 = @(t, vars) (params(3) - S0(t, vars));
    gam_u = @(t, vars) (C(t, vars)/(params(5) + C(t, vars)));
    gam_p = @(t, vars) (C(t, vars)/(params(6) + C(t, vars)));
    Sp = @(t, vars) min(6*params(3), B1 + 2*(B2 + B3 + B4) + 3*(B5 + B6 + B7 + B8) + 4*(B9 + B10 + B11) + 5*B12 + 6*B13);
    ...
    nu = @(t, vars) (k10(t, vars)*PP1);
    AMPA_bnd = @(t, vars) (2*(B2 + B3 + B4) ...
    + 6*(B5 + B6 + B7 + B8) ...
    + 12*(B9 + B10 + B11) ...
    + 20*B12 + 30*B13);
    ...
    eqs_det1 = @(t, Ca, B1, B2, B3, B4, B5, B6, B7, B8, B9, B10, B11, B12, B13, PP1, I1P) [
        0;
        6*params(11)*gam_u(t, vars)^2*B0(t, vars) - 4*params(11)*gam_u(t, vars)^2*B1 - chi(t, vars)*gam_u(t, vars)*B1 + nu(t, vars)*(2*(B2+B3+B4)-B1);
        params(11)*gam_u(t, vars)^2*B1 + chi(t, vars)*gam_u(t, vars)*B1 + nu(t, vars)*(3*(B5+B6+B7+B8)-2*B2) - 3*params(11)*gam_u(t, vars)^2*B2 - chi(t, vars)*gam_u(t, vars)*B2;
        2*params(11)*gam_u(t, vars)^2*B1 + nu(t, vars)*(3*(B5+B6+B7+B8)-2*B3) - 3*params(11)*gam_u(t, vars)^2*B3 - chi(t, vars)*gam_u(t, vars)*B3;
        params(11)*gam_u(t, vars)^2*B1 + nu(t, vars)*(3*(B5+B6+B7+B8)-2*B4) - 2*params(11)*gam_u(t, vars)^2*B4 - 2*chi(t, vars)*gam_u(t, vars)*B4;
        params(11)*gam_u(t, vars)^2*(B2+B3) + chi(t, vars)*gam_u(t, vars)*B2 + nu(t, vars)*(4*(B9+B10+B11)-3*B5) - 2*params(11)*gam_u(t, vars)^2*B5 - chi(t, vars)*gam_u(t, vars)*B5;
        params(11)*gam_u(t, vars)^2*(B2+B3) + 2*chi(t, vars)*gam_u(t, vars)*B4 + nu(t, vars)*(4*(B9+B10+B11)-3*B6) - params(11)*gam_u(t, vars)^2*B6 - 2*chi(t, vars)*gam_u(t, vars)*B6;
        params(11)*gam_u(t, vars)^2*(B2+2*B4) + chi(t, vars)*gam_u(t, vars)*B3 + nu(t, vars)*(4*(B9+B10+B11)-3*B7) - params(11)*gam_u(t, vars)^2*B7 - 2*chi(t, vars)*gam_u(t, vars)*B7;
        params(11)*gam_u(t, vars)^2*B3 + nu(t, vars)*(4*(B9+B10+B11)-3*B8) - 3*chi(t, vars)*gam_u(t, vars)*B8;
        params(11)*gam_u(t, vars)^2*B5 + chi(t, vars)*gam_u(t, vars)*(B6+B7) + nu(t, vars)*(5*B12-4*B9) - params(11)*gam_u(t, vars)^2*B9 - chi(t, vars)*gam_u(t, vars)*B9;
        params(11)*gam_u(t, vars)^2*(B5+B6) + chi(t, vars)*gam_u(t, vars)*(B7+B8) + nu(t, vars)*(5*B12-4*B10) - 2*chi(t, vars)*gam_u(t, vars)*B10;
        params(11)*gam_u(t, vars)^2*B7 + chi(t, vars)*gam_u(t, vars)*B6 + nu(t, vars)*(5*B12-4*B11) - 2*chi(t, vars)*gam_u(t, vars)*B11;
        params(11)*gam_u(t, vars)^2*B9 + chi(t, vars)*gam_u(t, vars)*(B9+2*B10+2*B11) + nu(t, vars)*(6*B13-5*B12) - chi(t, vars)*gam_u(t, vars)*B12;
        chi(t, vars)*gam_u(t, vars)*B12 - nu(t, vars)*6*B13;
        -params(16)*I1P*PP1 + params(17)*(params(19) - PP1);
        -params(16)*I1P*PP1 + params(17)*(params(19) - PP1) + vPKA_I1(t, vars)*(params(18)-I1P) - vCaN_I1(t, vars)*I1P;
    ];
    ...
    eqs_stoch = @(t, vars) [
        % 0;
        % lvl*(1/sqrt(params(35)*params(32)*pi*params(33)^2))*sqrt(6*params(11)*gam_u(t, vars)^2*S0(t, vars) + 4*params(11)*gam_u(t, vars)^2*B1 + chi(t, vars)*gam_u(t, vars)*B1 + nu(t, vars)*(2*(B2+B3+B4)+B1));
        % lvl*(1/sqrt(params(35)*params(32)*pi*params(33)^2))*sqrt(params(11)*gam_u(t, vars)^2*B1 + chi(t, vars)*gam_u(t, vars)*B1 + nu(t, vars)*(3*(B5+B6+B7+B8)+2*B2) + 3*params(11)*gam_u(t, vars)^2*B2 + chi(t, vars)*gam_u(t, vars)*B2);
        % lvl*(1/sqrt(params(35)*params(32)*pi*params(33)^2))*sqrt(2*params(11)*gam_u(t, vars)^2*B1 + nu(t, vars)*(3*(B5+B6+B7+B8)+2*B3) + 3*params(11)*gam_u(t, vars)^2*B3 + chi(t, vars)*gam_u(t, vars)*B3);
        % lvl*(1/sqrt(params(35)*params(32)*pi*params(33)^2))*sqrt(params(11)*gam_u(t, vars)^2*B1 + nu(t, vars)*(3*(B5+B6+B7+B8)+2*B4) + 2*params(11)*gam_u(t, vars)^2*B4 + 2*chi(t, vars)*gam_u(t, vars)*B4);
        % lvl*(1/sqrt(params(35)*params(32)*pi*params(33)^2))*sqrt(params(11)*gam_u(t, vars)^2*(B2+B3) + chi(t, vars)*gam_u(t, vars)*B2 + nu(t, vars)*(4*(B9+B10+B11)+3*B5) + 2*params(11)*gam_u(t, vars)^2*B5 + chi(t, vars)*gam_u(t, vars)*B5);
        % lvl*(1/sqrt(params(35)*params(32)*pi*params(33)^2))*sqrt(params(11)*gam_u(t, vars)^2*(B2+B3) + 2*chi(t, vars)*gam_u(t, vars)*B4 + nu(t, vars)*(4*(B9+B10+B11)+3*B6) + params(11)*gam_u(t, vars)^2*B6 + 2*chi(t, vars)*gam_u(t, vars)*B6);
        % lvl*(1/sqrt(params(35)*params(32)*pi*params(33)^2))*sqrt(params(11)*gam_u(t, vars)^2*(B2+2*B4) + chi(t, vars)*gam_u(t, vars)*B3 + nu(t, vars)*(4*(B9+B10+B11)+3*B7) + params(11)*gam_u(t, vars)^2*B7 + 2*chi(t, vars)*gam_u(t, vars)*B7);
        % lvl*(1/sqrt(params(35)*params(32)*pi*params(33)^2))*sqrt(params(11)*gam_u(t, vars)^2*B3 + nu(t, vars)*(4*(B9+B10+B11)+3*B8) + 3*chi(t, vars)*gam_u(t, vars)*B8);
        % lvl*(1/sqrt(params(35)*params(32)*pi*params(33)^2))*sqrt(params(11)*gam_u(t, vars)^2*B5 + chi(t, vars)*gam_u(t, vars)*(B6+B7) + nu(t, vars)*(5*B12+4*B9) + params(11)*gam_u(t, vars)^2*B9 + chi(t, vars)*gam_u(t, vars)*B9);
        % lvl*(1/sqrt(params(35)*params(32)*pi*params(33)^2))*sqrt(params(11)*gam_u(t, vars)^2*(B5+B6) + chi(t, vars)*gam_u(t, vars)*(B7+B8) + nu(t, vars)*(5*B12+4*B10) + 2*chi(t, vars)*gam_u(t, vars)*B10);
        % lvl*(1/sqrt(params(35)*params(32)*pi*params(33)^2))*sqrt(params(11)*gam_u(t, vars)^2*B7 + chi(t, vars)*gam_u(t, vars)*B6 + nu(t, vars)*(5*B12+4*B11) + 2*chi(t, vars)*gam_u(t, vars)*B11);
        % lvl*(1/sqrt(params(35)*params(32)*pi*params(33)^2))*sqrt(params(11)*gam_u(t, vars)^2*B9 + chi(t, vars)*gam_u(t, vars)*(B9+2*B10+2*B11) + nu(t, vars)*(6*B13+5*B12) + chi(t, vars)*gam_u(t, vars)*B12);
        % lvl*(1/sqrt(params(35)*params(32)*pi*params(33)^2))*sqrt(chi(t, vars)*gam_u(t, vars)*B12 + nu(t, vars)*6*B13);
        % lvl*(1/sqrt(params(35)*params(32)*pi*params(33)^2))*sqrt(+params(16)*I1P*PP1 + params(17)*(params(19) + PP1));
        % lvl*(1/sqrt(params(35)*params(32)*pi*params(33)^2))*sqrt(+params(16)*I1P*PP1 + params(17)*(params(19) + PP1) + vPKA_I1(t, vars)*(params(18)+I1P) + vCaN_I1(t, vars)*I1P);
        0;
        0;
        0;
        0;
