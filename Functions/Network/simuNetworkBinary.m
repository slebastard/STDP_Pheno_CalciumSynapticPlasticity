...
net.g=3.8;                      % Strength of inh/exc (inh connections -gJ)
...
for i=1:Iterations
    ...
    % Idea: change the rule so that W cannot vary while Ca2+ is above any
    % of the threshols
    %Once Ca2+ falls back to subthreshold levels, have W sample once from a Bernoulli whose parameter is rho_bas
    W = synSign.*transfer(rho, prot);
    
    % Steps:
    %1) Filter synapses who are about to hit basal calcium level (calcium fell below a NEW given threshold that must be below theta_d)
    % 2) Roll a Bernoulli per synapse, with each synapse having is own parameter
