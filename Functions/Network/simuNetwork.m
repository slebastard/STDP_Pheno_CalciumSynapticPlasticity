...
if 1
    ...
    net.synSignIn = 2*syn.J.*(net.WIn>0);
...
switch plt.all.raster
    case 1
        ...
        p = plot((1:simu.nIterTot), totActSnaps);
...
switch plt.spl.pres
    ...
    case 2
        ...
        for i=1:splSyn.n
            ...
            splSyn.pres(5*(i-1)+4,:)= maxCa.*(1/(syn.rho_max)).*splSyn.rho(i,:);
            splSyn.pres(5*(i-1)+5,:)= maxCa.*(1/syn.J).*splSyn.w(i,:);
        end
        fig.pres = figure('Name','NET_Summary','NumberTitle','off');
        imagesc(splSyn.pres)
        xticklabels(simu.dt.*xticks)
